...
class VerificationCameraActivity : AppCompatActivity() {
    ...
    private fun finishCapture() {
        ...
        saveToTempFile(averageEmbedding)
    ...
    private fun saveToTempFile(embedding: FloatArray) {
        try {
            val tempFile = File(cacheDir, "temp_face_embedding.txt")
            FileOutputStream(tempFile).use { fos ->
                val embeddingString = embedding.joinToString(",")
                fos.write(embeddingString.toByteArray())
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
